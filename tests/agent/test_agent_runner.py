...
def _get_metric_value(  # NOSONAR — receive-as-is integration scenario
    reader, metric_name, attributes=None
):
...
async def test_execute_single_turn_records_success(metrics_reader, tracing_exporter, registry):
    """Success path: span, llm_calls, and token counters all populate.

    Drives the read loop via direct calls to ``_handle_notification`` so we
    don't need a live codex process. Simulates the in-order delivery
    ``turn/started`` -> ``thread/tokenUsage/updated`` -> ``turn/completed``.
    """
    ...
    async def _fake_send_request(method, params):
        # Mimic the read loop firing the three notifications in order.
        await runner._handle_notification(
            {"method": "turn/started", "params": {"threadId": "test-thread-id"}}
        )
        await runner._handle_notification(
            {
    ...
    input_val = _get_metric_value(
        ...
        {"backend": "codex", "model": "gpt-5.4", "harness.nick": "spark-codex"},
    )
    assert input_val == 123
    ...
    assert output_val == 45


@pytest.mark.asyncio
async def test_execute_single_turn_records_late_token_usage(metrics_reader, registry):
    """Token counters still record if tokenUsage arrives after turn/completed.

    Exercises the bounded wait in ``_await_turn_token_usage`` without relying
    on wall-clock timing — the late delivery is coordinated by patching the
    helper so the notification fires only after ``_execute_single_turn`` has
    entered the wait, and the delivery task is awaited before assertions.
    """
    runner = _make_runner(registry=registry, nick="spark-codex", model="gpt-5.4")

    delivery_task: list[asyncio.Task] = []

    async def _deliver_late_usage():
        await runner._handle_notification(
            {
                "method": "thread/tokenUsage/updated",
                "params": {
    ...
    original_await = runner._await_turn_token_usage

    async def _coordinated_await(turn_id, **kwargs):
        # Schedule the late delivery exactly when the wait begins, then call
        # the real helper — its ``await asyncio.sleep(...)`` yields the loop,
        # which lets ``_deliver_late_usage`` run to completion before the
        # first ``pop`` is retried.
        delivery_task.append(asyncio.create_task(_deliver_late_usage()))
        return await original_await(turn_id, **kwargs)
    ...
    with patch.object(runner, "_send_request", side_effect=_fake_send_request):
        with patch.object(runner, "_await_turn_token_usage", side_effect=_coordinated_await):
            await runner._execute_single_turn("hello")

    # Explicitly await the delivery so a leftover task can't bleed into
    # teardown — addresses the un-awaited-task concern from review.
    assert delivery_task, "coordinated wait never scheduled the late delivery"
    await asyncio.wait_for(delivery_task[0], timeout=1.0)
...
@pytest.mark.asyncio
async def test_execute_single_turn_records_success_without_token_usage(metrics_reader, registry):
    """No tokenUsage notification: llm_calls still records, tokens stay 0.

    Simulates a codex version that does not emit
    ``thread/tokenUsage/updated``. The turn completes successfully and
    duration/call counters fire, but the token counters never see a data
    point with these attributes.
    """
    runner = _make_runner(registry=registry, nick="spark-codex", model="gpt-5.4")
        ...
        await runner._handle_notification(
            {
                ...
                "params": {
                    "threadId": "test-thread-id",
                    "turn": {"id": "turn-notokens", "status": "completed", "items": []},
                },
            }
        )
        return {"result": {}}
...
async def test_execute_single_turn_records_timeout(metrics_reader, registry):
    ...
    with patch.object(runner, "_send_request", side_effect=_fake_send_request):
        with patch(
            "codexd.agent.agent_runner.asyncio.timeout",
