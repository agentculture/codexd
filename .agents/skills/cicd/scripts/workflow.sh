...
# Codexd cicd workflow - thin layer over `agex pr` plus local extensions
# (`status`, `await`) for SonarCloud gating and triage flow.
#
# Subcommands:
#   lint                   `agex pr lint --exit-on-violation`. Same rules
#                          vendored in portability-lint.sh.
...
#
#   status <PR>            Codexd extension: pr-status.sh - SonarCloud
...
# agex's `--agent` flag accepts only claude-code|codex|copilot|acp.
# Codexd is a Codex-backed sibling, so default to codex.
AGEX_AGENT="${CODEXD_AGEX_AGENT:-codex}"
...
case "$cmd" in
    ...
    await)
        ...
        if [ -n "${CODEXD_PR_AWAIT_SECONDS:-}" ]; then
            echo "warning: CODEXD_PR_AWAIT_SECONDS is deprecated; prefer CODEXD_PR_AWAIT_WAIT." >&2
            echo "sleeping ${CODEXD_PR_AWAIT_SECONDS}s (legacy fixed-sleep) before agex pr read" >&2
            sleep "$CODEXD_PR_AWAIT_SECONDS"
            WAIT_ARGS=()
        else
            WAIT="${CODEXD_PR_AWAIT_WAIT:-1800}"
            WAIT_ARGS=(--wait "$WAIT")
        fi
