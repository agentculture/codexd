...
if TYPE_CHECKING:
    from codexd.harness.telemetry import HarnessMetricsRegistry

logger = logging.getLogger(__name__)


def _usage_dict_from_breakdown(last: dict) -> dict[str, int] | None:
    """Project codex's ``TokenUsageBreakdown`` onto the ``record_llm_call`` shape."""
    usage: dict[str, int] = {}
...
class CodexAgentRunner:
    ...
    def __init__(
        ...
        # Per-turn token-usage cache, populated by the
        # ``thread/tokenUsage/updated`` notification handler and drained
        # at the end of ``_execute_single_turn`` keyed by ``turnId``.
        self._token_usage_by_turn: dict[str, dict] = {}
        # The ``turn.id`` captured from the most recent ``turn/completed``
        # notification; ``_execute_single_turn`` uses it to look the
        # matching breakdown out of ``_token_usage_by_turn``.
    ...
    async def _handle_notification(self, msg: dict) -> None:
        ...
        if method == "turn/started":
            ...
            self._accumulated_text += params.get("delta", "")
        elif method == "thread/tokenUsage/updated":
            self._cache_token_usage(params)
        elif method == "turn/completed":
            await self._on_turn_completed(params)
        elif method in self._APPROVAL_METHODS:
            await self._auto_approve(msg)
        elif method == "error":
            await self._on_error_notification(params)

    def _cache_token_usage(self, params: dict) -> None:
        """Stash ``params['tokenUsage']['last']`` keyed by ``turnId``.

        codex-cli >=0.124 emits per-turn token breakdowns out of band from
        turn/completed (#27). Notifications for a foreign/stale thread are
        dropped so cross-thread payloads can't bleed into the active turn.
        """
        ...
        if isinstance(turn_id, str) and isinstance(last, dict):
            self._token_usage_by_turn[turn_id] = last

    async def _on_turn_completed(self, params: dict) -> None:
        """Capture the completed turn id and release the wait event."""
        self._busy = False
        if params.get("threadId") == self._thread_id:
    ...
    async def _on_error_notification(self, params: dict) -> None:
        """Log + propagate a codex `error` notification."""
    ...
    async def _execute_single_turn(self, text: str) -> None:
        ...
        # Reset before the turn so a stale value from a prior turn can't
        # leak into this turn's record_llm_call.
        ...
        try:
            if self._metrics is not None:
                usage_dict = await self._collect_turn_usage()
                record_llm_call(
                    self._metrics,
                    backend="codex",
                    model=self.model,
            ...
            # Any entries still in the cache are orphans from notifications
            # that arrived for turns we never drained (timeout/error, or
            # tokenUsage that landed after the bounded wait). Clearing per
            # turn keeps the cache bounded in long-running daemons.
            self._token_usage_by_turn.clear()

    async def _collect_turn_usage(self) -> dict[str, int] | None:
        """Build the ``record_llm_call`` usage dict for the completed turn."""
        turn_id = self._completed_turn_id
        if turn_id is None:
            return None
        last = await self._await_turn_token_usage(turn_id)
        if last is None:
            # Older codex (< 0.124) never emits thread/tokenUsage/updated;
            # this debug log lets operators diagnose the 100 ms bounded-wait
            # paid per turn without staring at unexplained latency.
            logger.debug("Codex turn %s: no tokenUsage notification arrived", turn_id)
            return None
        return _usage_dict_from_breakdown(last)

    async def _await_turn_token_usage(
        self,
        turn_id: str,
        *,
        attempts: int = 5,
        ...
        """Pop the cached ``TokenUsageBreakdown`` for ``turn_id``.

        codex emits ``thread/tokenUsage/updated`` and ``turn/completed`` as
        separate notifications, and the order between them is not documented.
        If the breakdown isn't already cached we yield to the read loop for
        up to ``attempts * tick_seconds`` seconds (default 100 ms) so an
        in-flight notification can be processed.
        """
        for _ in range(attempts):
            entry = self._token_usage_by_turn.pop(turn_id, None)
            if entry is not None:
                return entry
            await asyncio.sleep(tick_seconds)
        return self._token_usage_by_turn.pop(turn_id, None)
    ...
    async def _prompt_loop(self) -> None:
        ...
        try:
            ...
            raise  # NOSONAR — receive-as-is intentional swallow
