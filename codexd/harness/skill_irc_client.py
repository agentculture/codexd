"""IRC Skill Client — connects an agent backend to the culture daemon via Unix socket.

This module provides:
- ``SkillClient``: async client library for use from Python code
- CLI entry point: ``python -m codexd.clients.<backend>.skill.irc_client <subcommand> ...``
...
from codexd.harness.ipc import (
    MSG_TYPE_RESPONSE,
    MSG_TYPE_WHISPER,
    decode_message,
    encode_message,
...
class SkillClient:
    ...
    async def close(self) -> None:
        ...
        self._fail_pending("SkillClient closed")
    ...
    ) -> dict[str, Any]:  # NOSONAR — receive-as-is timeout API
        """Read recent messages from a channel buffer."""
        return await self._request("irc_read", channel=channel, limit=limit)

    async def irc_ask(  # NOSONAR — receive-as-is timeout API
        self, channel: str, question: str, timeout: int = 30  # NOSONAR — receive-as-is contract
    ) -> dict[str, Any]:
...
def _sock_path_from_env() -> str:
    ...
    if not nick:
        ...
        print("hint: CULTURE_NICK environment variable is not set", file=sys.stderr)
        sys.exit(2)
...
def _parse_ask_timeout(remaining: list[str]) -> tuple[int, list[str]]:
    ...
    Invalid values are user-input errors (AFI rubric exit code 1).
    """
    if "--timeout" in remaining:
        idx = remaining.index("--timeout")
        if idx + 1 >= len(remaining):
            print("hint: --timeout requires a value", file=sys.stderr)
            sys.exit(1)
        try:
            timeout = int(remaining[idx + 1])
        except ValueError:
            print(
                f"hint: --timeout value must be an integer, got {remaining[idx + 1]!r}",
                file=sys.stderr,
            )
            sys.exit(1)
        if timeout <= 0:
            print("hint: --timeout must be positive", file=sys.stderr)
            sys.exit(1)
