"""Shared config dataclasses + YAML helpers used by all four backends.

Each backend's ``codexd.clients.<backend>.config`` module subclasses
``Base{ServerConn,Supervisor,Agent,Telemetry,Daemon}Config`` to set its own
defaults (model, agent label, OTEL service name) and to add the few fields
that genuinely diverge (claude has the ``archived`` triple and a per-agent
``thinking`` knob; acp has ``acp_command``).

All file I/O, parsing, validation, sanitization, and the
archive_*/unarchive_* operations live here. Per-backend modules re-export
the surface declared in ``docs/api-stability.md``.
"""
...
from codexd.harness.attention import (
    AttentionConfig,
    Band,
    BandSpec,
    default_bands,
...
@dataclass
class BaseServerConnConfig:
    """IRC server connection settings (backend-agnostic core)."""
...
@dataclass
class BaseSupervisorConfig:
    """Supervisor sub-agent settings (backend-agnostic core)."""
...
class BaseAgentConfig:
    """Per-agent settings (backend-agnostic core)."""
...
class BaseDaemonConfig:
    """Top-level daemon configuration (backend-agnostic core).

    Subclasses override the ``server``/``supervisor``/``telemetry``/``agents``
    fields with backend-specific dataclasses and set ``_AGENT_CLS`` etc. so
    ``load_config`` can construct the right types.
    """

    server: BaseServerConnConfig = field(default_factory=BaseServerConnConfig)
    supervisor: BaseSupervisorConfig = field(default_factory=BaseSupervisorConfig)
    webhooks: WebhookConfig = field(default_factory=WebhookConfig)
    telemetry: BaseTelemetryConfig = field(default_factory=BaseTelemetryConfig)
    buffer_size: int = 500
    poll_interval: int = 60
    sleep_start: str = "23:00"
    sleep_end: str = "08:00"
    agents: list[BaseAgentConfig] = field(default_factory=list)
    attention: AttentionConfig = field(default_factory=AttentionConfig)

    # Subclasses set these to their concrete dataclass types.
    _SERVER_CLS: ClassVar[Type[BaseServerConnConfig]] = BaseServerConnConfig
    _SUPERVISOR_CLS: ClassVar[Type[BaseSupervisorConfig]] = BaseSupervisorConfig
    _AGENT_CLS: ClassVar[Type[BaseAgentConfig]] = BaseAgentConfig
    _TELEMETRY_CLS: ClassVar[Type[BaseTelemetryConfig]] = BaseTelemetryConfig

    def get_agent(self, nick: str) -> BaseAgentConfig | None:
...
# --- Band / attention helpers (pure functions, no per-backend state) ---
...
# --- YAML I/O (parameterized by daemon class) ---


def _filter_to_known_fields(raw: dict | None, cls: type) -> dict:
    """Return ``raw`` keyed only by fields the dataclass ``cls`` defines.

    Top-level blocks (server/supervisor/telemetry/webhooks) may contain
    backend-specific keys (e.g., claude's ``supervisor.thinking`` or
    ``server.archived``) that other backends' dataclasses don't accept.
    Without this filter, loading a mixed-backend ``agents.yaml`` under a
    backend that doesn't share those fields raises ``TypeError``. The
    raw YAML on disk is preserved by the raw-YAML mutation ops
    (archive/rename/remove); this filter is only for in-memory load.
    """
    if not raw:
        return {}
...
def load_config(path: str | Path, daemon_cls: Type[BaseDaemonConfig]) -> BaseDaemonConfig:
    """Load daemon config from a YAML file, returning a ``daemon_cls`` instance."""
    with open(path) as f:
        raw = yaml.safe_load(f) or {}

    server_cls = daemon_cls._SERVER_CLS
    supervisor_cls = daemon_cls._SUPERVISOR_CLS
    agent_cls = daemon_cls._AGENT_CLS
    telemetry_cls = daemon_cls._TELEMETRY_CLS

    server = server_cls(**_filter_to_known_fields(raw.get("server"), server_cls))
    supervisor = supervisor_cls(**_filter_to_known_fields(raw.get("supervisor"), supervisor_cls))
    webhooks = WebhookConfig(**_filter_to_known_fields(raw.get("webhooks"), WebhookConfig))
    telemetry = telemetry_cls(**_filter_to_known_fields(raw.get("telemetry"), telemetry_cls))

    legacy_poll_interval = raw.get("poll_interval", 60)
    attention = _build_attention_config(raw, legacy_poll_interval)

    agents = []
    known_agent_fields = {f.name for f in fields(agent_cls)}
    for agent_raw in raw.get("agents", []):
        # Accept both human-written ``attention:`` (YAML schema) and
        # round-tripped ``attention_overrides:`` (asdict() serialization);
        # prefer ``attention`` if both present. Strip unknown fields (e.g.
        # ``acp_command`` from ACP backend configs) so multi-backend configs
        # don't crash on load.
        ...
        agents.append(agent_cls(**filtered, attention_overrides=per_agent_attention))
...
def load_config_or_default(
    path: str | Path, daemon_cls: Type[BaseDaemonConfig]
) -> BaseDaemonConfig:
    """Load config from path, returning a default ``daemon_cls()`` if missing."""
    path = Path(path)
    if not path.exists():
        return daemon_cls()
    return load_config(path, daemon_cls)


def resolve_attention_config(
    daemon_cfg: BaseDaemonConfig, agent_cfg: BaseAgentConfig
) -> AttentionConfig:
...
def save_config(path: str | Path, config: BaseDaemonConfig) -> None:
...
def add_agent_to_config(
    ...
    daemon_cls: Type[BaseDaemonConfig],
    server_name: str | None = None,
) -> BaseDaemonConfig:
    """Add an agent to a config file, creating it if needed.

    If server_name is provided, updates config.server.name. Raises
    ValueError if an agent with the same nick already exists. Operates on
    raw YAML to preserve backend-specific fields.
    """
...
def rename_server(
    ...
    Returns (old_name, [(old_nick, new_nick), ...]). Operates on raw YAML
    to preserve backend-specific fields.
    """
    raw = _load_raw_yaml(path)
    server = raw.get("server", {})
    old_name = server.get("name", BaseServerConnConfig().name)
...
# --- Archive ops (currently only re-exported by the claude backend) ---
...
def archive_agent(
    ...
    Raises ValueError if the agent is not found. Operates on raw YAML to
    preserve backend-specific fields.
    """
    raw = _load_raw_yaml(path)
    for agent_raw in raw.get("agents", []):
        if agent_raw.get("nick") == nick:
...
def unarchive_agent(
    ...
    Raises ValueError if not found or not currently archived. Operates on
    raw YAML to preserve backend-specific fields.
    """
    raw = _load_raw_yaml(path)
    for agent_raw in raw.get("agents", []):
...
def archive_server(
    ...
    Returns a list of archived agent nicks. Operates on raw YAML to
    preserve backend-specific fields.
    """
    raw = _load_raw_yaml(path)
...
def unarchive_server(
    ...
    Returns a list of unarchived agent nicks. Operates on raw YAML to
    preserve backend-specific fields.
    """
    raw = _load_raw_yaml(path)
